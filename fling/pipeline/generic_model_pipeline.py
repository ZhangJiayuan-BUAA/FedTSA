...
def generic_model_pipeline(args: dict, seed: int = 0) -> None:
    ...
    best_test_iid = 0.
    torch.save(group.server.glob_dict,
               os.path.join(args.other.logging_path, 'model_iid{:.4f}.ckpt'.format(best_test_iid)))
    ...
    for i in range(args.learn.global_eps):
        ...
        if i % args.other.test_freq == 0 and "after_aggregation" in args.learn.test_place:
            ...
            if test_result["test_acc"] > best_test_iid:
                os.remove(os.path.join(args.other.logging_path, 'model_iid{:.4f}.ckpt'.format(best_test_iid)))
                best_test_iid = test_result["test_acc"]
                torch.save(group.server.glob_dict,
                           os.path.join(args.other.logging_path, 'model_iid{:.4f}.ckpt'.format(best_test_iid)))
